...
Today the board's past is one overwrite-on-read file: `state.json` remembers only "last time," and `summary` reports what changed since. M4b gives the board a **memory**: timestamped, versioned snapshots that accumulate as a side effect of normal use, listable and diffable across any two points — "what changed this week?", "when did #42 leave Building?", "what did the board look like before the cleanup?"
...
### In scope
- **`lib/snapshots.mjs`** — the snapshot store: write (dedup + prune), list, read-by-ref, pure `diffSnapshots`.
- **`summary` piggyback** — non-fatal snapshot write after summary's existing work.
- **`snapshot take ["label"]` / `snapshot list` / `snapshot diff <ref> [<ref2>]`** verbs + CLI (loadConfig path).
...
.github-boards/snapshots/snapshot-<stamp>.json      (gitignored dir, M1's home)
   ...
   PRUNE: after each successful write, delete oldest beyond `keep` (default 50)
        │
snapshot take ["label"]       manual save-point (same dedup/prune; label optional)
...
|---|---|---|
| **`lib/snapshots.mjs`** | Snapshot store; owns `.github-boards/snapshots/`. | `writeSnapshot(dir, items, {label?, keep?})` → `{path, skipped:false}` \| `{skipped:true, reason}` · `listSnapshots(dir)` → `[{file, takenAt, label, count}]` newest-first · `readSnapshot(dir, ref)` → snapshot object · `diffSnapshots(prevItems, currItems)` → diff (pure) · `resolveRef(refs, ref)` (pure ref→file resolution, exported for tests) |
| **`board-manager.mjs`** | `summary` piggyback; `snapshotTake(label, ctx)`, `snapshotList(ctx)`, `snapshotDiff(refA, refB, ctx)` verbs; CLI `snapshot <take\|list\|diff>`. | `snapshotDiff(refA, refB?, ctx)` — `refB` omitted → live board via `engine.listItems()` |
...
- **Retention:** default 50; `config.snapshots.keep` (positive integer; anything else → default). Prune deletes strictly the valid-snapshot-named files beyond newest-N; foreign files in the directory are never touched.
...
1. **`lib/snapshots.mjs` unit:** write→list→read round-trip; dedup (identical items → skipped, no second file; changed items → new file); prune (N+3 distinct writes with keep=N → oldest deleted, foreign file untouched); ref resolution (`latest`, date prefix, `~N`, out-of-range/unknown → legible errors); `diffSnapshots`: each bucket isolated + a combined multi-bucket case + label-order insensitivity + same-items → all empty.
2. **Verb tests (mock engine):** summary piggyback — one snapshot on changed board, none on unchanged, injected store failure → summary still succeeds with the suffix; `snapshotTake` stores the label; `snapshotDiff` two refs and ref-vs-live; say lines.
