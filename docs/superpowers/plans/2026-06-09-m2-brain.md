...
| `tests/mapper-contract.test.mjs` | **New** | Structural smoke: contract doc + SKILL wiring (Task 6). |
| `tests/rules-backcompat.test.mjs` | **New** | `rules` block back-compat: absent → defaults, present → pass-through (Task 7). |
...
**Status-vocabulary note:** M1's ledger candidate `status` was `candidate | promoted | dismissed`. M2 extends it with `mapped | needs-decision | merged | split`. This is an intentional, additive extension (no M1 code depends on the closed set). The authoritative full set is `candidate | mapped | needs-decision | merged | split | dismissed | promoted`. (The spec's §3/§7 inline lists were non-exhaustive — omitting `split` — and have been corrected to match.)

**Deferred to M3 — the `needsDecision` resolution loop:** M2 *surfaces* needs-decision questions (records status `needs-decision`, returns them from `map record`) but does NOT implement the answer→re-record path (no `map resolve` sub-verb, no `--remap`). Re-mapping a settled candidate belongs with M3's interactive promotion loop and is out of scope here — a conscious deferral, not an omission.
...
And add `session: null, proposals: null` to the `out` initializer object.
...
Expected: PASS (2 tests). (`loadConfig` in scripts/lib/config.mjs returns `{ ...cfg, preset, routing }` — it spreads all raw keys, so `rules` already passes through. This test just confirms it; no loader change is needed.)

- [ ] **Step 3: Document the block in `board.example.json`.** `pullCadence` is currently the **last** property (no trailing comma). Add a comma to it, then append `rules` as the new last property. The result should read:
...
i.e. change the existing `"pullCadence": "session-start"` line to end with a comma, then add the `"rules": { ... }` object with **no** trailing comma after its closing brace (it is now the final property). Verify the file still parses: `node -e "JSON.parse(require('fs').readFileSync('board.example.json','utf8')); console.log('valid')"`.
...
- [ ] **Step 1: Run the full suite AND confirm the nested tests are collected**

Run: `npm test`
Expected: all pass (M1 + M2 unit/verb/contract/rules/score tests; the gated simulation test skipped). The `node --test tests/**/*.test.mjs` script relies on Node's native test-runner glob, which needs **Node ≥ 21** (the dev box is Node 22) to match the new nested `tests/simulation/` files. Confirm: (a) the total test count rose from M1's 145 by the M2 tests, and (b) `tests/simulation/score.test.mjs` is among those run. If on Node < 21 (no glob), run the nested file explicitly — `node --test tests/simulation/score.test.mjs` — and treat the M2 score tests as a separate command.
...
- **Spec coverage:** harness (prepare/record) → Tasks 2,4,5. validate fail-closed → Task 3. ruleset (knobs) → Tasks 1,7 + contract Task 6. proposal schema (kind/lane/owner/split/merge/needsDecision) → Tasks 3,4 + contract 6. idempotency (content-hash, settled-skip, index-salted split ids) → Task 4. ambiguity/escalation → contract Task 6 (escalation is a contract instruction, no harness code — confirmed) + needs-decision *surfacing* Tasks 4,5. simulation harness → Tasks 8,9. enrich-ledger-only / never-board → Tasks 4,5. ✓ **Conscious deferral:** the needsDecision *resolution* loop (answer→re-record) is out of scope, moved to M3 (see Scope note); M2 ships no `--remap`. ✓
